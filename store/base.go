...
func (b *Store[T]) CountByGroup(ctx context.Context, wm map[string]interface{}, groupBy string) (map[string]int64, error) {
	db := b.DB(ctx)
...
func (b *Store[T]) UniqueIndex() {
	b.Model = new(T)
	var mv interface{} = b.Model
	if tb, ok := mv.(Table); ok {
		b.name = tb.TableName()
	} else {
		b.name = reflect.TypeOf(b.Model).Elem().Name()
	}
...
func (b *Store[T]) ListPageWhere(ctx context.Context, where map[string]any, pageNum, pageSize int, order string) ([]*T, int64, error) {
...
//
//// Transaction 执行事务
//func (b *CommitStore[T]) Transaction(ctx context.Context, f func(context.Context) error) error {
//	if b.IsTxCtx(ctx) {
//		return f(ctx)
//	}
//
